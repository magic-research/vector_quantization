...
class BertEmbeddings(nn.Module):
    ...
    def __init__(
        self, vocab_size, hidden_size, max_position_embeddings, dropout=0.1
    ):
        super().__init__()
...
class MaskedGenerativeEncoderViT(nn.Module):
    ...
    def forward_encoder(self, x):
        ...
        while True:
            ...
            if token_drop_mask.sum(
            ) == bsz * num_dropped_tokens and token_all_mask.sum(
            ) == bsz * num_masked_tokens:
                break
            else:
                print("Rerandom the noise!")
        # print(mask_rate, num_dropped_tokens, num_masked_tokens, token_drop_mask.sum(dim=1), token_all_mask.sum(dim=1))
        token_indices[token_all_mask.nonzero(as_tuple=True)
                      ] = self.mask_token_label
        ...
        token_drop_mask = torch.cat([
            torch.zeros(token_indices.size(0), 1).cuda(), token_drop_mask
        ],
                                    dim=1)
        token_all_mask = torch.cat([
            torch.zeros(token_indices.size(0), 1).cuda(), token_all_mask
        ],
                                   dim=1)
        ...
        input_embeddings_after_drop = input_embeddings[token_keep_mask.nonzero(
            as_tuple=True
    ...
    def forward_decoder(self, x, token_drop_mask, token_all_mask):
        ...
        x_after_pad[(1 - token_drop_mask).nonzero(as_tuple=True)
    ...
    def forward_loss(self, gt_indices, logits, mask):
        ...
        loss = (loss * mask[:, 1:]
                ).sum() / mask[:, 1:].sum()  # mean loss on removed patches
        return loss
